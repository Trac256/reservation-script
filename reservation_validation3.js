(function () {
    ...
    function applyValidation() {
        ...
        if (fullNameInput) {
            ...
            fullNameInput.setAttribute("placeholder", "Μόνο γράμματα (Ελληνικά και Αγγλικά) και έως 2 ονόματα και 2 επώνυμα");
        }
        ...
        if (addressInput && submitButton && form) {
        ...
        console.log("✅ reservation_validation.js applied successfully!");
    }

    // Εφαρμογή της λογικής μόλις φορτώσει η σελίδα
    ...
    // Παρακολούθηση αλλαγών στο DOM με MutationObserver
    const observer = new MutationObserver(function (mutations, observerInstance) {
        const addressInput = document.getElementById('reservation_address');
        if (addressInput) {
            applyValidation();
            observerInstance.disconnect(); // Σταμάτα την παρακολούθηση μετά την εφαρμογή
        }
    });
